...
import { ShowTowingOrders } from "../../../redux/API/ordersSlice";
import { Tag } from "primereact/tag";

const TowingOrderTable = (props) => {
  const dispatch = useDispatch();
  const { loading, userTowings, totalItems } = useSelector(
    (state) => state.orders
  );
  ...
  useEffect(() => {
    ...
    dispatch(ShowTowingOrders(info));
  }, []);
  ...
  return (
    <div className="datatable">
      {loading && <LoadingFS />}
      <div className="">
        <DataTable value={userTowings} tableStyle={{ minWidth: "50rem" }}>
          <Column
            align="center"
            header={headers[0]}
...
export default TowingOrderTable;
